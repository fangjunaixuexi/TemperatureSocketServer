...
public class EnvironmentalDao {
	...
	/*
	 * System.out.println("连接数据库成功，发送sql语句"); String
	 * sql="select * from equipment where serial_number=?"; Equipment
	 * equipment=null; try { equipment=jdbcTemplate.queryForObject(sql, new
	 * Object[]{serial_number},new EquipmentRowMapper()); } catch
	 * (EmptyResultDataAccessException e) { return null; } return equipment; }
	 */
			 ...
			 while (rs.next()) {  
		           String rowData =null;
		            for (int i = 1; i <= columnCount; i++) {  
		                rowData=rs.getString(md.getColumnName(i));  
		            }  
		            list.add(rowData);  
			 }
		} catch (Exception e) {
	...
	public int insertEnvironmental_monitoring(Environmental_monitoring environmental_monitoring) {
		...
		int result = jdbcTemplate.update(sql,
				new Object[] { environmental_monitoring.getEnviromental_id(), environmental_monitoring.getTemperature(),
						environmental_monitoring.getHumidity(), environmental_monitoring.getIllumination(),
